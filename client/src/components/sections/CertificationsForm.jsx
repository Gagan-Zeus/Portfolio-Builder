      ...
      {items.map((item, i) => (
        <div key={i} className="rounded-xl p-5 space-y-4" style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.08)' }}>
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-slate-700">Certification {i + 1}</span>
            <button onClick={() => remove(i)} className="text-red-500 hover:text-red-500 p-1"><Trash2 size={14} /></button>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-slate-500 mb-1">Certification Name</label>
              <input className="input-field" placeholder="AWS Solutions Architect" value={item.name} onChange={e => update(i, 'name', e.target.value)} />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">Issuing Organization</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">Credential URL</label>
